...
import type { Project,ReportItem,ReportItemType,Tag,WeeklyReport } from '@zhoubao/shared';
import { AlertTriangle,ArrowLeft,ArrowRight,CalendarDays,Check,ChevronDown,Eye,GripVertical,ImagePlus,Pencil,Plus,RefreshCcw,Trash2 } from 'lucide-react';
...
export function ReportPage({user}:{user:User}){
  ...
  const addMutation=useMutation({
    mutationFn:async({type,projectId}:{type:ReportItemType;projectId:string|null})=>{
      ...
      return api(`/api/reports/${data.id}/items`,{method:'POST',body:JSON.stringify({type,projectId,contentMd:'',tagIds:[]})});
    },
    onSuccess:()=>{qc.invalidateQueries({queryKey:['report',year,week]});qc.invalidateQueries({queryKey:['report-weeks',year]});}
  });

  if(report.isLoading||reportWeeks.isLoading||projects.isLoading||tags.isLoading)return <PageFrame title="周报"><Loading/></PageFrame>;
  ...
  return <div className="page report-page">
    ...
    <div className="report-sections">{sections.map((type,index)=><ReportSection key={type} index={index} type={type} items={data.items.filter(item=>item.type===type)} report={data} projects={activeProjects} tags={tags.data!.tags} onAdd={projectId=>addMutation.mutate({type,projectId})}/>)}</div>
  </div>;
}
...
function ReportSection({type,items,report,projects,tags,onAdd,index}:{type:ReportItemType;items:ReportItem[];report:WeeklyReport;projects:Project[];tags:Tag[];onAdd:(projectId:string|null)=>void;index:number}){
  const qc=useQueryClient();
  ...
  return <section className="report-section project-table-section" style={{'--section-index':index} as React.CSSProperties}>
    <div className="section-heading"><div><span className={`section-index section-${type}`}>{String(index+1).padStart(2,'0')}</span><div><h2>{sectionLabels[type]}</h2><p>{sectionHints[type]}</p></div></div><button className="button ghost" onClick={()=>onAdd(null)}><Plus size={17}/>添加</button></div>
    {items.length?<DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={dragEnd}><SortableContext items={items.map(item=>item.id)} strategy={verticalListSortingStrategy}><div className="project-report-groups">{groups.map(group=><ProjectReportGroup key={group.key} group={group} report={report} tags={tags} onAdd={()=>onAdd(group.project?.id??null)}/>)}</div></SortableContext></DndContext>:<button className="section-empty" onClick={()=>onAdd(null)}><Plus size={18}/><span>添加一条{sectionLabels[type]}</span></button>}
  </section>;
}

function ProjectReportGroup({group,report,tags,onAdd}:{group:ProjectItemGroup;report:WeeklyReport;tags:Tag[];onAdd:()=>void}){
  const qc=useQueryClient();
  ...
  return <section className="project-report-group">
    ...
    <div className="project-group-rows">{group.items.map((item,index)=><ReportItemRow key={item.id} item={item} sequence={index+1} report={report} tags={tags}/>)}<button className="project-row-add" onClick={onAdd}><Plus size={13}/>添加一条</button></div>
  </section>;
}

function summarizeMarkdown(content:string){return content.replace(/!\[([^\]]*)\]\([^)]+\)/g,'[图片] $1').replace(/[#*_`>]/g,'').replace(/\s+/g,' ').trim();}

function ReportItemRow({item,report,tags,sequence}:{item:ReportItem;report:WeeklyReport;tags:Tag[];sequence:number}){
  const qc=useQueryClient();
  ...
  useEffect(()=>{setContent(item.contentMd);setTagIds(item.tags.map(tag=>tag.id));version.current=item.version;},[item.id,item.version]);
  useEffect(()=>()=>window.clearTimeout(clickTimer.current),[]);
  const save=useMutation({
    mutationFn:()=>api<ReportItem>(`/api/report-items/${item.id}`,{method:'PATCH',body:JSON.stringify({contentMd:content,tagIds,expectedVersion:version.current})}),
  ...
  useEffect(()=>{if(initial.current){initial.current=false;return;}const timer=window.setTimeout(()=>save.mutate(),800);return()=>window.clearTimeout(timer);},[content,tagIds.join(',')]);
  ...
  return <article ref={sortable.setNodeRef} style={style} className={`report-table-row ${status==='conflict'?'item-conflict':''}`}>
    <div className="report-row-main">
      ...
      <div className="row-actions"><span className={`save-status ${status}`}>{status==='saving'?<RefreshCcw size={12} className="spin"/>:status==='saved'?<Check size={12}/>:status==='conflict'?'冲突':'失败'}</span><button className="icon-button" onClick={()=>{setDetailEditing(false);setDetailsOpen(true);}} aria-label="打开详情"><Eye size={15}/></button><button className="icon-button danger" onClick={()=>{if(confirm('删除这条周报内容？'))remove.mutate();}} aria-label="删除"><Trash2 size={15}/></button></div>
    </div>
...
function TagPicker({tags,selected,onChange}:{tags:Tag[];selected:string[];onChange:(ids:string[])=>void}){
  ...
  return <div className="tag-picker inline-tags">{selectedTags.map(tag=><button type="button" className="inline-tag" key={tag.id} onClick={()=>onChange(selected.filter(id=>id!==tag.id))} title={`移除标签 ${tag.name}`}><span className="tag-dot" style={{background:tag.color}}/>{tag.name}<b aria-hidden="true">×</b></button>)}<button type="button" className="tag-add" onClick={()=>setOpen(!open)} aria-expanded={open}><Plus size={12}/>{selectedTags.length?'标签':'添加标签'}</button>{open&&<div className="tag-popover">{tags.length?tags.map(tag=><label key={tag.id}><input type="checkbox" checked={selected.includes(tag.id)} onChange={()=>onChange(selected.includes(tag.id)?selected.filter(id=>id!==tag.id):[...selected,tag.id])}/><span className="tag-dot" style={{background:tag.color}}/>{tag.name}</label>):<span className="muted">请先在项目页旁的标签区域创建标签</span>}</div>}</div>;
}
